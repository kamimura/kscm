((lambda ()
  ...
  (define (string-append . strings)
    (define (iter1 strings)
      (if (null? strings)
          '()
          (cons (string->list (car strings))
                (iter1 (cdr strings)))))
    ...
    ((lambda (primitive-branch after-call)
       ((lambda (compiled-linkage)
          (append-instruction-sequences
           (make-instruction-sequence
            '(proc) '()
            (list '|if (proc->t == PROC) { goto | primitive-branch '|;}|
                  '|if (proc->t != CPROC) {|
                  '|fprintf(stderr, "unknown procedure type\x5c;n");|
                  '|exit(1);}|))
           (parallel-instruction-sequences
  ...
  (if (= (length commands) 2)
      ((lambda ()
         ...
         (if (= n 0)
             ((lambda ()
                (define n (system (string-append "./" name)))
                (if (= n 0)
                    "done (runksc)"
                    (error "Error(runksc):" n))))
             (error "Error(compile):" n))
         ))
      (error "usage: runksc name"))
  ))
