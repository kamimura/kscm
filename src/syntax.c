...
static void load_write(Object o, FILE *stream) { write(stream, "load"); }

#include "env.h"    // def_var_val
#include "symbol.h" // symbol_new
void syntax_init() {

  fn_of_obj_file of_obj_file_vs[] = {
      quote_write, lambda_write, define_write, if_write,   set_write,
      begin_write, and_write,    or_write,     load_write, NULL};
  Type ts[] = {QUOTE,     LAMBDA,  DEFINE, IF,  SET,
               BEGIN_OBJ, AND_OBJ, OR_OBJ, LOAD};
